...
与 `import` 声明不同，`#include` 指令后不能有分号。另外，不要忘记前面的符号 `#`
:::
...
例如，输入 $77$ 英寸，程序应该输出 $6$ 英尺 $5$ 英寸。提示用户输入一个单位是英寸的整数值，在进行转换输出结构。$1$ 英尺等于 $12$ 英寸
...
int main()
{
    // foot: 英尺
    // inch: 英寸
    const int inchs_per_foot {12};  // 1 英尺 = 12 英寸
    int feet;
    int inchs;
    std::cout << "输入整数长度(单位英寸): ";
    std::cin >> inchs;
    feet = inchs / inchs_per_foot;  // 从 inches 中获取 12 的倍数
    inchs %= inchs_per_foot;        // 获取剩余的 inches
    std::cout << std::format("{} 英寸等于 {} 英尺 {} 英寸\n", feet * inchs_per_foot + inchs, feet, inchs);
    return 0;
}
```
...
int main()
{
    ...
    std::cout << "输入圆的半径: ";
    std::cin >> radius;
    double areas {radius * radius * std::numbers::pi};
    std::cout << std::format("半径为 {:.2f} 圆的面积为 {:.2f}\n", radius, areas);
    return 0;
}
```
...
如果知道自己与树之间的距离 $d$ 以及自己与树梢连线与水平线的夹角 $\alpha$，则树的高度 $h = d \times \tan{(\alpha)}$。编写程序，输入距离 $d$ 和夹角 $\alpha$ 计算树的高度
...
int main()
{
    ...
    std::cout << "输入夹角: ";
    std::cin >> alpha;
    double height {distance * std::tan(alpha)};
    std::cout << std::format("数的高度为: {}\n",height);
    return 0;
}
```
...
$$
BMI = \frac{\text{weight}(\text{体重：kg})}{\text{height}^2(\text{身高: m})}
$$
...
int main()
{
    ...
    std::cout << std::format("身高为 {} 体重为 {} 的 BMI 为 {:.1f}\n", height, weight, bmi);
    return 0;
}
```
